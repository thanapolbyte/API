...
def api_POST(externalId):
    ...
    if res.status == 201 :
            query2 = "Update Infobip_Event_Sale_Detail set status = 'Y', Update_date_time = getdate() where Infobip_Sale_Code + '_' + Customer_ID = " +"'" + str(externalId) +"'"
            cursor = conn1.cursor()
            cursor.execute(query2)
            # conn1.commit()
    else:
            # query3 = "Update Infobip_Event_Sale_Detail set status = 'E', Update_date_time = getdate() where Infobip_Sale_Code + '_' + Customer_ID = " +"'" + str(externalId) +"'"
            # cursor = conn1.cursor()
...
def filename():
    sql_query = pd.read_sql_query(f''' 
                              select Infobip_Sale_Code + '_' + Customer_ID as salecode_personid FROM Infobip_Event_Sale_Detail where status ='N'
...
def query(salecode_personid):
    ...
    query = "select itemRuleBase,VoucherCode,VoucherName,itemDepartment,itemCategory,itemSKU,Total_Qty,Store_Code,channel,Total_Amount,Brand,point,transactionDate,Receipt_Number FROM Infobip_Event_Sale_Detail where Infobip_Sale_Code + '_' + Customer_ID = " +"'" + str(salecode_personid) +"'"
