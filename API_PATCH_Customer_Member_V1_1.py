...
def query(id):
    ...
    conn = conn_string
    cursor = conn.cursor()
    query = "SELECT Last_Updated_Date,Last_Updated_By,Card_Expiry_Date,CAST([point] as int) as point,All_Accumulate_Purchase as All_Accumulate_Purchase,[Accumulate_Purchase],[Member_Level],[Customer_id],[Card_Date] FROM Infobip_Update_Customer_v1_1  where  Customer_id =" +"'" + str(id) +"'"
...
def API_PATCH(externalId):
    ...
    cursor.execute('''
                INSERT INTO [Infobip_customer_log_v1_1] (Customer_id,datas,status,reason,decode,interface_dates)
                VALUES (?,?,?,?,?,?)
                ''',
                str(externalId),
                str(payload),
    ...
    if res.status == 200 :
            query2 = "Update Infobip_Update_Customer_v1_1 set Status = 'Y', Update_Date_time = getdate() where [Customer_id] = " +"'" + str(externalId) +"'"
            # cursor = conn.cursor()
            cursor.execute(query2)
            # conn1.commit()
    else:
            query3 = "Update Infobip_Update_Customer_v1_1 set Status = 'E', Update_Date_time = getdate() where [Customer_id] = " +"'" + str(externalId) +"'"
            # cursor = conn.cursor()
...
def filename():
    sql_query = pd.read_sql_query(f''' 
                              select Customer_id  FROM Infobip_Update_Customer_v1_1 where Status = 'N' ORDER BY Last_Updated_Date DESC
                              ''' 
                              ,sql_connect())
    df = pd.DataFrame(sql_query)
