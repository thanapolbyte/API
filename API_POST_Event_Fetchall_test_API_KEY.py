...
def api_POST(externalId):
    ...
    cursor = conn1.cursor()
    cursor.execute('''
                INSERT INTO MinorPOS.dbo.Infobip_event_20_log (Types,External_ID,Saleline_id,Customer_id,datas,status,reason,decode,insert_dates)
                VALUES (?,?,?,?,?,?,?,?,?)
                ''',
                'Event',
                # str(externalId),
    ...
    if res.status == 201 :
            query2 = "Update Infobip_Event_Sale_Detail_20 set status = 'Y', Update_date_time = getdate() where Infobip_Sale_Code + '_' + Customer_ID = " +"'" + str(externalId) +"'"
            cursor = conn1.cursor()
            cursor.execute(query2)
            # conn1.commit()
    else:
            query3 = "Update Infobip_Event_Sale_Detail_20 set status = 'E', Update_date_time = getdate() where Infobip_Sale_Code + '_' + Customer_ID = " +"'" + str(externalId) +"'"
            cursor = conn1.cursor()
...
def filename():
    sql_query = pd.read_sql_query(f''' 
                              select Infobip_Sale_Code + '_' + Customer_ID as salecode_personid FROM Infobip_Event_Sale_Detail_20 where status ='N'
