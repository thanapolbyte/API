...
def query(id):
    ...
    conn = conn_string
    cursor = conn.cursor()
    query = "SELECT Last_Updated_Date,Last_Updated_By,Card_Expiry_Date,CAST([point] as int) as point,All_Accumulate_Purchase as All_Accumulate_Purchase,[Accumulate_Purchase],[Member_Level],[Customer_id],[Card_Date] FROM xxx_Infobip_Update_Customer  where  Customer_id =" +"'" + str(id) +"'"
...
def API_PATCH(externalId):
    ...
    cursor.execute('''
                INSERT INTO [xxx_Infobip_customer_log] (Customer_id,datas,status,reason,decode,interface_dates)
                VALUES (?,?,?,?,?,?)
                ''',
                str(externalId),
                # str(payload2),
    ...
    if res.status == 200 :
            query2 = "Update xxx_Infobip_Update_Customer set Status = 'Y', Update_Date_time = getdate() where [Customer_id] = " +"'" + str(externalId) +"'"
            # cursor = conn.cursor()
            ...
            # conn1.commit()
            # cursor.execute('''xxx_Infobip_Update_Customer_Bad_Request''')
            pass
...
def filename():
    sql_query = pd.read_sql_query(f''' 
                              select Customer_id  FROM xxx_Infobip_Update_Customer where Status = 'N'
                              ''' 
                              ,sql_connect())
    df = pd.DataFrame(sql_query)
